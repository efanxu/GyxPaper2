...
while($stack.Count -gt 0){
  ...
  foreach($p in $children){
    ...
    if($i.Length -ge 10MB){try{$sha=(Get-FileHash -LiteralPath $p -Algorithm SHA256).Hash}catch{$sha='HASH_ERROR'}}
    $rows.Add([pscustomobject]@{RelativePath=$r;SizeBytes=[int64]$i.Length;Extension=$e;LastWriteTime=$i.LastWriteTime.ToString('o');Classification=$c;PlannedToCommit=$dcs[0];PlannedLFS=$dcs[1];ExclusionReason=$dcs[2];SHA256=$sha;SensitiveReview=$s})
  }
}
...
$ex=@('# path<TAB>size_bytes<TAB>sha256<TAB>classification<TAB>reason')+@($rows|? PlannedToCommit -eq 'No'|%{'{0}`t{1}`t{2}`t{3}`t{4}'-f $_.RelativePath,$_.SizeBytes,$_.SHA256,$_.Classification,$_.ExclusionReason})
