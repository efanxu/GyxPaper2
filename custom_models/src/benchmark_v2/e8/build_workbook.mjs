...
for (const [name, rawRows] of Object.entries(payload.sheets)) {
  ...
  const used = sheet.getRange(`A1:${lastColumn}${normalized.length}`);
  used.values = normalized;
  used.format = { font: { name: "Arial", size: 10 }, verticalAlignment: "top" };
  const header = sheet.getRange(`A1:${lastColumn}1`);
  header.format = {
    fill: "#17365D", font: { name: "Arial", size: 10, bold: true, color: "#FFFFFF" },
    wrapText: true, borders: { preset: "outside", style: "thin", color: "#8EA9C1" },
  };
  used.format.autofitColumns();
  used.format.autofitRows();
  used.format.columnWidth = 22;
  sheet.getRange(`A1:A${normalized.length}`).format.columnWidth = 30;
