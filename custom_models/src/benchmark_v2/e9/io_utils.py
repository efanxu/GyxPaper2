from __future__ import annotations

import csv
import json
import math
import os
from pathlib import Path
from typing import Any, Iterable, Mapping


def read_json(path: str | Path) -> Any:
    return json.loads(Path(path).read_text(encoding="utf-8"))


def write_json(path: str | Path, payload: Any) -> Path:
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
    temporary = target.with_name(target.name + ".tmp")
    temporary.write_text(json.dumps(payload, ensure_ascii=False, indent=2, allow_nan=False) + "\n", encoding="utf-8")
    os.replace(temporary, target)
    return target


def write_csv(path: str | Path, rows: Iterable[Mapping[str, Any]], fields=None) -> Path:
    target = Path(path)
    materialized = list(rows)
    target.parent.mkdir(parents=True, exist_ok=True)
    columns = list(fields or (materialized[0].keys() if materialized else []))
    temporary = target.with_name(target.name + ".tmp")
    with temporary.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=columns, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(materialized)
    os.replace(temporary, target)
    return target


def read_metrics(path: str | Path) -> dict[int, dict[str, float]]:
    rows: dict[int, dict[str, float]] = {}
    with Path(path).open("r", encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            horizon = int(row["horizon"])
            values = {}
            for metric in ("Score", "MAE", "RMSE", "R2"):
                raw = row.get(metric)
                if raw in (None, ""):
                    raise ValueError(f"Missing {metric} at H{horizon}: {path}")
                value = float(raw)
                if not math.isfinite(value):
                    raise ValueError(f"Non-finite {metric} at H{horizon}: {path}")
                values[metric] = value
            rows[horizon] = values
    if set(rows) != {3, 6, 10}:
        raise ValueError(f"Metrics must contain exactly H3/H6/H10: {path}")
    return rows


def forbidden_source(path: str | Path) -> str | None:
    from .constants import FORBIDDEN_SOURCE_TOKENS
    normalized = str(path).replace("\\", "/").lower()
    return next((token for token in FORBIDDEN_SOURCE_TOKENS if token.replace("\\", "/") in normalized), None)


def file_identity(path: str | Path) -> dict[str, Any] | None:
    candidate = Path(path)
    if not candidate.is_file():
        return None
    stat = candidate.stat()
    return {"path": str(candidate.resolve()), "size_bytes": stat.st_size, "mtime_ns": stat.st_mtime_ns}
