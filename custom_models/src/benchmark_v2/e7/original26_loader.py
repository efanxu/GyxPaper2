...
def load_original26(path: str | Path) -> tuple[dict[str, Original26Model], dict[str, Any]]:
    ...
    for model_id in EXTERNAL_MODEL_IDS:
        ...
        statuses = {str(row["run_status"]).upper() for row in model_rows}
        protocols = {str(row["protocol_status"]).upper() for row in model_rows}
        metric_statuses = {str(row["metric_status"]).upper() for row in model_rows}
        if len(run_ids) != 1 or len(groups) != 1 or len(sources) != 1:
            errors.append(f"{model_id}:NON_UNIQUE_IDENTITY")
            continue
        if statuses != {"COMPLETED"} or protocols != {"PASS"} or metric_statuses != {"OK"}:
            errors.append(f"{model_id}:FORMAL_STATUS_INVALID")
            continue
        metrics = {
            int(row["Horizon"]): {metric: finite_number(row[metric]) for metric in METRICS}
            for row in model_rows
        }
        parameter_values = {row.get("parameter_count") for row in model_rows}
        parameter_count = next(iter(parameter_values)) if len(parameter_values) == 1 else None
    ...
    audit = {
        ...
        "sheets": [MAIN_SHEET, DETAIL_SHEET],
        "main_columns": main_headers,
        "detail_columns": detail_headers,
        "selected_model_ids": list(models),
