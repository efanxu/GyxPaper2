...
def _artifact_runtime() -> tuple[Path, Path]:
    node = os.environ.get("E7_ARTIFACT_NODE")
    modules = os.environ.get("E7_ARTIFACT_NODE_MODULES")
    if not node or not modules:
        raise RuntimeError(
            "ARTIFACT_TOOL_RUNTIME_MISSING: set E7_ARTIFACT_NODE and "
            "E7_ARTIFACT_NODE_MODULES from Codex load_workspace_dependencies."
        )
