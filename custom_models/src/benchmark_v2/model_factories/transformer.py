...
def create_model(*, config: Mapping[str, Any], protocol: Mapping[str, Any], **_: Any) -> Any:
    from ..models.transformer import create_model as _create_model
...
def resolve_config(protocol: Mapping[str, Any], *, run_mode: str = "formal", ma_window: int | None = None) -> dict[str, Any]:
    from ..configs.transformer import resolve_transformer_config

    return resolve_transformer_config(protocol, run_mode=run_mode)


def create_adapter(*, protocol: Mapping[str, Any], config: Mapping[str, Any], **_: Any) -> Any:
    from ..adapters.e2_a import TransformerAdapter

    return TransformerAdapter(protocol, label_len=int(config["label_len"]))
