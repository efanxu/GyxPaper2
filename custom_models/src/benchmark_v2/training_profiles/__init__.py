...
class TrainingBatchProfile:
    ...
    def identity(self) -> dict[str, Any]:
        return {
            "base_benchmark_protocol_hash": self.payload[
                "base_benchmark_protocol_hash"
            ],
            "training_batch_profile_id": self.profile_id,
            "training_batch_profile_hash": self.profile_hash,
            "train_batch_size": self.train_batch_size,
            "val_batch_size": self.val_batch_size,
            "test_batch_size": self.test_batch_size,
            "gradient_accumulation_steps": int(
                self.payload["gradient_accumulation_steps"]
            ),
            "effective_train_batch_size": int(
                self.payload["effective_train_batch_size"]
            ),
...
def _validate(payload: dict[str, Any]) -> None:
    ...
    if payload["profile_id"] not in PROFILE_ALLOWLIST:
        raise ProtocolError(
            f"Training batch profile is not allowlisted: {payload['profile_id']}"
        )
    expected = {
        "train_batch_size": 4,
        "val_batch_size": 4,
...
def load_training_profile(
    profile_id: str | None,
) -> TrainingBatchProfile | None:
    if profile_id in (None, ""):
        return None
    if profile_id not in PROFILE_ALLOWLIST:
        raise ProtocolError(
            f"Unknown training batch profile {profile_id!r}; "
            f"allowed={PROFILE_ALLOWLIST}"
        )
    ...
    _validate(payload)
    return TrainingBatchProfile(payload=payload, profile_hash=stable_hash(payload))
...
def resolved_batch_sizes(
    protocol: Mapping[str, Any], profile_id: str | None
) -> dict[str, int]:
    profile = load_training_profile(profile_id)
    if profile is None:
        return {
            "train": int(protocol["train_batch_size"]),
            "val": int(protocol["val_batch_size"]),
            "test": int(protocol["test_batch_size"]),
        }
    return {
        "train": profile.train_batch_size,
        "val": profile.val_batch_size,
        "test": profile.test_batch_size,
    }
