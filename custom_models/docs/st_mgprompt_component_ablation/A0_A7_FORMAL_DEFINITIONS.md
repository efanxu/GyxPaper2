...
| A6 | Single-direction Cross Fusion |
| A7 | w/o MS-MG-DWU |

A6 keeps `fusion_mode=cross`, Coarse/Macro-to-Fine cross-attention, the learned adaptive gate, residual connections, LayerNorm, and the complete A0 parameterization. Its only formal semantic difference from A0 is `disable_reverse_cross: false -> true`, so Fine-to-Coarse attention is not executed and the Coarse branch receives no reverse update. A5 remains distinct: it keeps both directions and only shortens the Fine-to-Coarse context from 24 to 6 steps.
