...
Changing feature, target, or mask values cannot change graph identity because edge construction accepts only canonical node IDs, x/y, and protocol constants. A location file byte change changes location identity and graph-protocol identity. If only unused elevation changes, node metadata and protocol identity change, while base matrix hashes and graph-bundle content hash remain unchanged.
