...
import numpy as np
import torch
from benchmark_v2.adapters import BaselineScalerContext, NodeSharedGRUAdapter, StatisticalBaselineAdapter
from benchmark_v2.configs import resolve_gru_config, resolve_moving_average_config, resolve_persistence_config
...
def scalers():
    input_scaler = StandardScaler(mean=np.arange(16, dtype=np.float64) * 10.0, std=np.arange(1, 17, dtype=np.float64))
    target_scaler = StandardScaler(mean=np.asarray(500.0, dtype=np.float64), std=np.asarray(125.0, dtype=np.float64))
...
def batch_from_physical(physical_power):
    ...
    power_index = protocol['ordered_input_features'].index(protocol['input_power_column'])
    x[..., power_index] = (physical - float(input_scaler.mean[power_index])) / float(input_scaler.std[power_index])
    target_raw = torch.full((b, n, 10), 700.0)
    target = (target_raw - float(target_scaler.mean)) / float(target_scaler.std)
    batch = BenchmarkBatch(x=x, target=target, mask=torch.ones(b, n, 10, dtype=torch.bool), target_raw_or_inverse_transform=target_raw, sample_ids=list(range(b)), window_end_indices=list(range(b)), node_ids=list(range(n)), metadata={'contains_future_target': False})
...
class StatisticalBaselineTests(unittest.TestCase):
    ...
    def test_persistence_physical_scaler_roundtrip_and_parameters(self):
        ...
        runtime = build_model_runtime('persistence', load_protocol(), run_mode='smoke', input_scaler=input_scaler, target_scaler=target_scaler)
        ...
        self.assertTrue(torch.allclose(restored, expected, atol=1e-05))
        ...
        self.assertNotEqual(runtime.scaler_context.input_power_mean, runtime.scaler_context.target_mean)
        self.assertNotEqual(runtime.scaler_context.input_power_std, runtime.scaler_context.target_std)

    def test_persistence_rejects_nonfinite_last_visible(self):
        batch, input_scaler, target_scaler = batch_from_physical(torch.ones(2, 144, 3))
    ...
    def test_moving_average_known_values_scaler_and_formal_window(self):
        ...
        batch, input_scaler, target_scaler = batch_from_physical(physical)
        runtime = build_model_runtime('moving_average', load_protocol(), run_mode='smoke', input_scaler=input_scaler, target_scaler=target_scaler, ma_window=4)
        output = runtime.adapter(runtime.model, batch)
        restored = runtime.inverse_target(output.prediction)
        self.assertTrue(torch.allclose(restored, torch.full((1, 1, 10), 250.0)))
        formal = create_ma({'run_mode': 'formal', 'ma_window': 144}, load_protocol())
...
class NodeSharedGRUTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.runtime = build_model_runtime('gru', self.protocol, run_mode='smoke')

    def make_batch(self, b=2, t=144, n=4):
        x = torch.randn(b, t, n, 16)
        target = torch.randn(b, n, 10)
        mask = torch.ones(b, n, 10, dtype=torch.bool)
        return BenchmarkBatch(x=x, target=target, mask=mask, sample_ids=list(range(b)), window_end_indices=list(range(b)), node_ids=list(range(n)), metadata={'contains_future_target': False})
    ...
    def test_shape_structure_and_fixed_parameter_count(self):
        ...
        self.assertEqual(sum((isinstance(module, torch.nn.GRU) for module in self.runtime.model.modules())), 1)
        self.assertEqual(sum((isinstance(module, torch.nn.Linear) for module in self.runtime.model.modules())), 1)
        self.assertFalse(any((isinstance(module, (torch.nn.ModuleList, torch.nn.Embedding)) for module in self.runtime.model.modules())))
    ...
    def test_masked_backward_gradients_and_invalid_positions(self):
        ...
        self.assertTrue(any((parameter.grad is not None for parameter in self.runtime.model.gru.parameters())))
        self.assertIsNotNone(self.runtime.model.prediction_head.weight.grad)
        self.assertTrue(torch.equal(output.prediction.grad[:, :, 3:], torch.zeros_like(output.prediction.grad[:, :, 3:])))
        if torch.cuda.is_available():
            cuda_runtime = build_model_runtime('gru', self.protocol, run_mode='smoke')
            cuda_batch = self.make_batch().x.cuda()
            with torch.autocast('cuda', dtype=torch.float16):
                raw = cuda_runtime.model.cuda()(cuda_batch.permute(0, 2, 1, 3).contiguous().reshape(8, 144, 16))
    ...
    def test_configs_record_frozen_optimizer_and_semantics(self):
        ...
        self.assertEqual(resolve_persistence_config(self.protocol)['optimizer'], None)
