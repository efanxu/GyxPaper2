...
def historical_args(model_id):
    common = dict(task_name='long_term_forecast', seq_len=144, label_len=48, pred_len=10, enc_in=16, c_out=16, d_model=16, n_heads=4, e_layers=1, d_layers=1, d_ff=32, dropout=0.0, embed='timeF', freq='h')
...
class E2BModelTests(unittest.TestCase):
    ...
    def test_allowlist_is_exactly_eighteen_and_registry_counts(self):
        ...
        self.assertEqual(stats['available_locally_verified'] + stats['available_with_hardware_preflight'], 26)
    ...
    def test_shapes_backward_target_mask_independence_and_strict_reload(self):
        for model_id in MODEL_IDS:
            with self.subTest(model_id=model_id):
                ...
                loss.backward()
                self.assertTrue(any((parameter.grad is not None for parameter in runtime.model.parameters() if parameter.requires_grad)))
    ...
    def test_node_independence_and_parameter_count_does_not_depend_on_n(self):
        for model_id in MODEL_IDS:
            with self.subTest(model_id=model_id):
                ...
                self.assertTrue(torch.allclose(out_single, out_double, atol=1e-06))
        ...
        for model_id in MODEL_IDS:
            with self.subTest(model_id=model_id):
                ...
                with self.assertRaisesRegex(ContractError, 'future'):
                    runtime.adapter(runtime.model, make_batch(), future_observed_covariates=torch.ones(1, 10, 16))
    ...
    def test_micn_original_decoder_mark_length_mismatch(self):
        ...
        with self.assertRaisesRegex(RuntimeError, '154.*58'):
            model(x, torch.zeros(2, 144, 4), torch.zeros(2, 58, 16), torch.zeros(2, 58, 4))

    def test_micn_fixed_decoder_and_mark_lengths(self):
        runtime = self.build('micn')
        trace = runtime.adapter(runtime.model, make_batch()).aux['decoder_trace']
