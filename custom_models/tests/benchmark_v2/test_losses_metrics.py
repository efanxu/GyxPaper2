...
class LossMetricsTests(unittest.TestCase):
    def test_masked_losses_zero_valid_and_backward(self):
        p = torch.tensor([[[1.,2.,3.],[2.,3.,4.]]], requires_grad=True)
        y = torch.zeros_like(p); m = torch.tensor([[[1,0,1],[1,1,0]]], dtype=torch.bool)
        loss = get_loss("masked_mse")(p,y,m); self.assertIsNotNone(loss); loss.backward(); self.assertIsNotNone(p.grad)
