...
class ArtifactTests(unittest.TestCase):
    def test_path_escape_and_duplicate_rejection(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td) / "root"
            run = safe_run_dir(root, "suite", "run"); run.mkdir(parents=True)
            with self.assertRaises(ArtifactError): safe_run_dir(root, "suite", "run")
            with self.assertRaises(ArtifactError): safe_run_dir(root, "..", "outside")
    ...
    def test_failed_and_incomplete_are_not_completed(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            run2 = Path(td) / "run2"; run2.mkdir(); write_status(run2, status="COMPLETED", run_mode="smoke", artifact_profile="SMOKE")
            with self.assertRaises(ArtifactError): validate_run(run2)
