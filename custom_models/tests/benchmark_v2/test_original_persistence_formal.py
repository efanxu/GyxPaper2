...
class OriginalPersistenceFormalFixtureTests(unittest.TestCase):
    def test_tiny_formal_evaluate_only_completes_with_non_trainable_artifacts(self):
        ...
        node_count = 4
        inputs = rng.normal(
            size=(timestamp_count, node_count, 16)
        ).astype(np.float32)
        inputs[..., 15] = rng.uniform(
            10.0, 1000.0, size=(timestamp_count, node_count)
        ).astype(np.float32)
        targets = rng.uniform(
            10.0, 1000.0, size=(timestamp_count, node_count)
        ).astype(np.float32)
        ...
        timestamps = pd.date_range(
            "2026-01-01", periods=timestamp_count, freq="10min"
        ).tolist()
        provider = SDWPFDataProvider.from_arrays(
            inputs,
            targets,
            mask,
            ...
            runtime.effective_config.update(
                {
                    ...
                    "source_revision": {
                        "git_commit": kwargs["source_revision"],
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with (
                ...
                patch.object(
                    ...
                    side_effect=apply_fixture_scope_identity,
                ),
                patch(
                    "benchmark_v2.hardware_preflight.preflight_identity",
                    return_value={"scope_id": CURRENT_SCOPE26_ID},
                ),
            ):
                stdout = io.StringIO()
                with redirect_stdout(stdout):
                    exit_code = main(
                        [
                            ...
                            "persistence",
                            "--input-path",
                            str(Path(temp) / "fixture-input.parquet"),
                            "--target-path",
                            str(Path(temp) / "fixture-target.parquet"),
                            "--output-root",
                            str(output_root),
                            "--run-id",
                            "Persistence_bs4_seed2026",
                            "--device",
                            "cpu",
                            "--training-profile",
            ...
            self.assertEqual(
                effective["training_batch_profile_id"],
                "uniform_train_batch4_v1",
            )
            ...
            for horizon in (3, 6, 10):
                ...
                for metric in ("MAE", "RMSE", "R2", "Score"):
                    self.assertTrue(
                        ...
                        f"non-finite H{horizon} {metric}: {metrics}",
                    )
                self.assertGreater(int(metrics["valid_target_count"]), 0)
            self.assertEqual(
                effective["test_batch_size"],
                4,
            )
