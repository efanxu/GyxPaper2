...
def subset_batch(batch, indices):
    return BenchmarkBatch(
        ...
        sample_ids=[batch.sample_ids[i] for i in indices],
        window_end_indices=[batch.window_end_indices[i] for i in indices],
...
class E2DModelTests(unittest.TestCase):
    ...
    def test_loader_registry_and_source_identity(self):
        self.assertEqual(len(ALLOWED_TSLIB_MODELS), 18)
        self.assertEqual(ALLOWED_TSLIB_MODELS[-3:], MODEL_IDS)
        expected = {
        ...
        for model_id, digest in expected.items():
            self.assertEqual(resolve_tslib_source(model_id).source_sha256, digest)
        ...
        for model_id in MODEL_IDS:
            with self.subTest(model_id=model_id):
                ...
                second = runtime.adapter(
                    runtime.model,
                    changed,
                    test_target=torch.randn_like(batch.target) * 1000,
                )
    ...
    def test_batch_companion_permutation_and_single_item_invariance(self):
        for model_id in MODEL_IDS:
            with self.subTest(model_id=model_id):
                ...
                single = runtime.adapter(
                    runtime.model, subset_batch(ab, [0])
                ).prediction
                permuted = runtime.adapter(
                    runtime.model, subset_batch(ab, [1, 0])
                ).prediction
                self.assertTrue(torch.allclose(out_ab[0], out_ac[0], atol=1e-6))
                self.assertTrue(torch.allclose(out_ab[0], single[0], atol=1e-6))
                self.assertTrue(torch.allclose(out_ab[0], permuted[1], atol=1e-6))
    ...
    def test_cross_node_isolation_and_parameter_count_independent_of_n(self):
        for model_id in MODEL_IDS:
            with self.subTest(model_id=model_id):
                ...
                self.assertTrue(
                    torch.allclose(original[:, 1], modified[:, 1], atol=1e-6)
                )
                self.assertFalse(torch.equal(original[:, 0], modified[:, 0]))
                self.assertEqual(
                    runtime.parameter_count,
                    self.build(model_id).parameter_count,
                )
                self.assertFalse(
                    any(
    ...
    def test_crossformer_geometry_and_attention_gradients(self):
        ...
        groups = (
            "enc_value_embedding.value_embedding.weight",
            "router",
            "time_attention.query_projection.weight",
            "dim_sender.query_projection.weight",
            "merge_layer.linear_trans.weight",
            "self_attention.router",
            "cross_attention.query_projection.weight",
            "linear_pred.weight",
            "dec_pos_embedding",
        )
        for token in groups:
    ...
    def test_msgnet_period_graph_fft_and_all_scale_gradients(self):
        ...
        self.assertTrue(
            all(row["fft_dtype"] == "torch.complex64" for row in trace["traces"])
        )
        output.prediction.square().mean().backward()
    ...
    def test_hardware_preflight_identity_exact(self):
        for model_id in MODEL_IDS:
    ...
    def test_crossformer_skips_mandatory_preflight_and_failures_start_no_formal(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            calls.clear()
            failing = lambda argv, check=False: (
