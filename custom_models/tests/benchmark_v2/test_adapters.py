...
class AdapterTests(unittest.TestCase):
    ...
    def test_bad_b_n_and_multivariable_output_rejected(self):
        ...
        class Bad(torch.nn.Module):
            ...
            def forward(self, x):
                return torch.zeros(3, 10)
        with self.assertRaises(ContractError):
            NodeSharedAdapter()(Bad(), b)

        class Variables(torch.nn.Module):

            def forward(self, x):
                return torch.zeros(8, 10, 16)
        with self.assertRaises(ContractError):
            NodeSharedAdapter()(Variables(), b)
    ...
    def test_native_and_graph_context(self):
        ...
        class Native(torch.nn.Module):
            ...
            def forward(self, x):
                return torch.zeros(x.shape[0], x.shape[2], 10)
        ...
        with self.assertRaises(ContractError):
            graph(Native(), b)
        context = GraphContext(tuple(range(4)), {'source': 'explicit'}, False, False, {'normalization': 'none'})
