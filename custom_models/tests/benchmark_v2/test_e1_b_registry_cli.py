...
class E1BRegistryTests(unittest.TestCase):
    def test_registry_final_counts_and_e1_a_entries_unchanged(self):
        ...
        self.assertEqual(
            registry.get("persistence").runtime_status,
            "AVAILABLE_NON_TRAINABLE",
        )
        self.assertEqual(
            registry.get("moving_average").runtime_status,
            "AVAILABLE_NON_TRAINABLE",
        )
    ...
    def test_registry_list_show_and_protocol_check_do_not_import_tslib(self):
        ...
        for argv in (
            ...
            self.assertFalse(
                any(
                    name.startswith("_benchmark_v2_tslib_")
