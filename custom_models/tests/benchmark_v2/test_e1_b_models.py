...
import torch
from benchmark_v2.adapters import DLinearAdapter, LightTSAdapter, SegRNNAdapter, TiDEAdapter
from benchmark_v2.checkpointing import CheckpointManager
from benchmark_v2.configs import resolve_dlinear_config, resolve_lightts_config, resolve_segrnn_config, resolve_tide_config
...
class E1BModelContractTests(unittest.TestCase):
    ...
    def test_targets_and_masks_are_not_model_inputs(self):
        for model_id in MODEL_IDS:
            with self.subTest(model_id=model_id):
                ...
                first = runtime.adapter(runtime.model, first_batch).prediction.detach()
                second = runtime.adapter(runtime.model, second_batch).prediction.detach()
    ...
    def test_tide_zero_placeholders_and_nonzero_future_covariates_rejected(self):
        ...
        with self.assertRaisesRegex(ContractError, 'future observed'):
            runtime.adapter(runtime.model, batch, future_observed_covariates=torch.ones(2, 10, 1))

    def test_segrnn_original_failure_and_protocol_compatible_shape(self):
        model_class, _ = load_tslib_model_class('segrnn')
        original = SimpleNamespace(seq_len=144, pred_len=10, enc_in=16, d_model=512, dropout=0.1, task_name='long_term_forecast', seg_len=96)
        ...
        with self.assertRaisesRegex(RuntimeError, 'size of tensor a \\(0\\)'):
        ...
        self.assertEqual(tuple(fixed.model(torch.randn(8, 144, 16), None, None, None).shape), (8, 10, 16))

    def test_frozen_configs_are_not_test_selected(self):
        resolvers = (resolve_dlinear_config, resolve_lightts_config, resolve_tide_config, resolve_segrnn_config)
