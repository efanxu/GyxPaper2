...
from benchmark_v2.configs import resolve_agcrn_config, resolve_graph_wavenet_config, resolve_mtgnn_config, resolve_stid_config
...
from benchmark_v2.models.graph_models import AdaptiveGraphConv, DilatedInception, DirectedGraphConstructor
from benchmark_v2.models.graph_models.adaptive_common import E3_C_POLICIES
...
def node_batch(batch_size=1, seed=2026):
    ...
    return BenchmarkBatch(x=torch.randn(batch_size, 144, 134, 16, generator=generator), target=torch.randn(batch_size, 134, 10, generator=generator), mask=torch.ones(batch_size, 134, 10, dtype=torch.bool), sample_ids=list(range(batch_size)), window_end_indices=list(range(batch_size)), node_ids=list(range(1, 135)), metadata={'contains_future_target': False, 'history_end_timestamp': ['2021-01-04 12:00:00' for _ in range(batch_size)], 'history_end_time_of_day_id': [72 for _ in range(batch_size)], 'history_end_day_of_week_id': [0 for _ in range(batch_size)], 'timezone_policy': 'SOURCE_NAIVE_UNCHANGED'})

def clone_batch(batch):
    return BenchmarkBatch(x=batch.x.clone(), target=batch.target.clone(), mask=batch.mask.clone(), sample_ids=list(batch.sample_ids), window_end_indices=list(batch.window_end_indices), node_ids=list(batch.node_ids), split=batch.split, metadata=copy.deepcopy(batch.metadata))
...
class TestE3CRegistryConfigAndPolicy(unittest.TestCase):
    ...
    def test_registry_count_unique_ids_and_allowlist(self):
        ...
        for model_id in MODELS:
            matches = [entry for entry in registry.list() if entry.canonical_id == model_id]
    ...
    def test_configs_are_frozen_before_smoke(self):
        ...
        self.assertEqual((gwn['residual_channels'], gwn['skip_channels'], gwn['blocks'], gwn['layers_per_block'], gwn['diffusion_order']), (32, 256, 4, 2, 2))
        mtg = resolve_mtgnn_config(self.protocol, run_mode='smoke')
        self.assertEqual((mtg['subgraph_size'], mtg['node_dim'], mtg['layers']), (20, 40, 3))
        self.assertEqual(mtg['num_split'], 1)
        self.assertFalse(mtg['node_sampling'])
        agc = resolve_agcrn_config(self.protocol, run_mode='smoke')
        self.assertEqual((agc['embed_dim'], agc['rnn_units'], agc['num_layers'], agc['cheb_order']), (10, 64, 2, 2))
        stid = resolve_stid_config(self.protocol, run_mode='smoke')
        self.assertEqual((stid['embed_dim'], stid['node_dim'], stid['time_of_day_dim'], stid['day_of_week_dim'], stid['num_layers']), (32, 32, 32, 32, 3))

    def test_physical_adaptive_and_identity_policies(self):
        expected = {'graph_wavenet': (True, ['P_forward', 'P_reverse']), 'mtgnn': (False, []), 'agcrn': (False, []), 'stid': (False, [])}
        for model_id, (uses_physical, names) in expected.items():
            ...
            self.assertEqual(identity['adaptive_graph_policy'], E3_C_POLICIES[model_id]['adaptive_graph_policy'])
        stid_model = build_model_runtime('stid', self.protocol, run_mode='smoke').model
        self.assertFalse(any(('graph' in type(module).__name__.casefold() for module in stid_model.modules())))
...
class TestNativeNodeIdentityAndLeakage(unittest.TestCase):
    ...
    def test_target_mask_invariance_and_future_inputs_rejected(self):
        ...
        for model_id in MODELS:
            ...
            for key in ('future_observed_covariates', 'future_weather', 'future_calendar_marks', 'future_timestamps'):
                with self.subTest(model=model_id, key=key), self.assertRaises(ContractError):
                    runtime.adapter.prepare_model_inputs(original, **{key: torch.ones(1)})
    ...
    def test_batch_companion_permutation_and_single_equivalence(self):
        ...
        for model_id in MODELS:
            ...
            for key in ('history_end_timestamp', 'history_end_time_of_day_id', 'history_end_day_of_week_id'):
            ...
            self.assertTrue(torch.allclose(together[:1], alone, atol=1e-06), model_id)
            self.assertTrue(torch.allclose(together, swapped.flip(0), atol=1e-06), model_id)

class TestGraphWaveNet(unittest.TestCase):

    def test_receptive_field_alignment_support_branches_and_gradients(self):
        runtime = build_model_runtime('graph_wavenet', load_protocol(), run_mode='smoke')
        model = runtime.model
        batch = node_batch()
        ...
        self.assertEqual(model.last_shape_trace['forecast_anchor'], 'last_valid_causal_temporal_position')
        prediction.square().mean().backward()
        for name, parameter in (('nodevec1', model.nodevec1), ('nodevec2', model.nodevec2), ('filter', model.filter_convs[0].weight), ('gate', model.gate_convs[0].weight), ('diffusion', model.graph_convs[0].projection.weight), ('skip', model.skip_convs[0].weight), ('end', model.end_conv_2.weight)):
        ...
        self.assertTrue(torch.allclose(adjacency.sum(1), torch.ones(134), atol=1e-06))
...
class TestMTGNN(unittest.TestCase):
    ...
    def test_constructor_inception_mixhop_and_gradients(self):
        ...
        prediction.square().mean().backward()
        for name, parameter in (('emb1', model.graph_constructor.emb1.weight), ('emb2', model.graph_constructor.emb2.weight), ('inception-k2', model.filter_convs[0].branches[0].weight), ('inception-k7', model.filter_convs[0].branches[3].weight), ('forward-mixhop', model.forward_props[0].projection.weight), ('reverse-mixhop', model.reverse_props[0].projection.weight), ('end', model.end_conv_2.weight)):
            assert_finite_nonzero(self, parameter, name)
...
class TestAGCRN(unittest.TestCase):
    ...
    def test_dagg_napl_two_basis_recurrent_head_and_gradients(self):
        ...
        prediction.square().mean().backward()
        for name, parameter in (('node-embedding', model.node_embeddings), ('layer0-gate-pool', model.cells[0].gate.weights_pool), ('layer0-candidate-pool', model.cells[0].candidate.weights_pool), ('layer1-gate-pool', model.cells[1].gate.weights_pool), ('head', model.horizon_head.weight)):
            assert_finite_nonzero(self, parameter, name)
        model.eval()
        with torch.no_grad():
            adaptive = model(batch.x)
            identity = model(batch.x, adjacency_override=torch.eye(model.num_nodes))
...
class TestSTID(unittest.TestCase):
    ...
    def test_timestamp_identity_ranges_anchor_change_and_future_independence(self):
        ...
        self.assertEqual(runtime.adapter.prepare_model_inputs(future_changed)[1].tolist(), [72])
        anchor_changed = clone_batch(batch)
        anchor_changed.metadata['history_end_timestamp'] = ['2021-01-05 01:20:00']
        anchor_changed.metadata['history_end_time_of_day_id'] = [8]
        anchor_changed.metadata['history_end_day_of_week_id'] = [1]
        _, changed_tod, changed_dow = runtime.adapter.prepare_model_inputs(anchor_changed)
    ...
    def test_embeddings_mlp_gradients_and_cross_node_isolation(self):
        ...
        prediction = runtime.adapter(model, batch).prediction
        prediction.square().mean().backward()
        for name, parameter in (('series', model.time_series_embedding.weight), ('node', model.node_embedding), ('tod', model.time_of_day_embedding.weight), ('dow', model.day_of_week_embedding.weight), ('mlp0', model.encoder[0].linear1.weight), ('head', model.regression_head.weight)):
            assert_finite_nonzero(self, parameter, name)
        model.eval()
