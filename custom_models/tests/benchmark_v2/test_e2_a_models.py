...
class E2AModelTests(unittest.TestCase):
    ...
    def test_shapes_backward_and_no_target_or_mask_leakage(self):
        for model_id in MODEL_IDS:
            with self.subTest(model_id=model_id):
                ...
                loss.backward()
                self.assertTrue(any((p.grad is not None for p in runtime.model.parameters() if p.requires_grad)))
    ...
    def test_itransformer_has_sixteen_feature_tokens(self):
        ...
        self.assertEqual(output.aux['variable_token_semantics'], 'features_within_one_turbine')
    ...
    def test_timexer_endogenous_exogenous_and_future_rejection(self):
        ...
        with self.assertRaisesRegex(ContractError, 'future'):
            runtime.adapter(runtime.model, make_batch(), future_exogenous=torch.ones(2, 10, 15))
        with self.assertRaisesRegex(ContractError, 'future exogenous'):
            runtime.adapter(runtime.model, make_batch(), future_exogenous=torch.zeros(2, 10, 15))
