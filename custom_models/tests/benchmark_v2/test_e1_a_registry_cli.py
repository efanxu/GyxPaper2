...
class E1ARegistryAndCLITests(unittest.TestCase):
    def test_registry_counts_factories_and_lazy_import(self):
        for name in [
            "benchmark_v2.models.persistence",
            "benchmark_v2.models.moving_average",
            "benchmark_v2.models.gru",
        ]:
        ...
        available = [
            ...
            if str(entry.runtime_status).startswith("AVAILABLE_")
        ]
        ...
        self.assertEqual(
            sum(entry.supports_non_trainable for entry in entries), 2
        )
        self.assertNotIn("benchmark_v2.models.gru", sys.modules)
        protocol = __import__(
            "benchmark_v2.protocol", fromlist=["load_protocol"]
        ).load_protocol()
        ...
        self.assertEqual(
            registry.get("dlinear").runtime_status, "AVAILABLE_TRAINABLE"
        )
        self.assertEqual(
    ...
    def test_train_non_trainable_and_unavailable_fail_closed(self):
        for model_id, expected, terminal_message in [
            (
                "persistence",
                "deterministic non-trainable baseline",
            ...
            self.assertIn(terminal_message, stream.getvalue())
